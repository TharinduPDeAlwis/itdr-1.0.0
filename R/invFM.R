...
#' @details
#' \loadmathjax 
#' Let \mjseqn{(\textbf{y}_i,\textbf{x}_i), i =1,\cdots,n}, be a random sample, and assume that the dimension of \mjseqn{S_{E(\textbf{Z} | \textbf{Y})}} is known to be \emph{d}.
#' Then, for a random finite sequence of \mjseqn{\boldsymbol{\omega}_j \in \mathbb{R}^p}, \mjseqn{j=1,\cdots,t} compute \mjseqn{\widehat{\boldsymbol{\psi}}(\boldsymbol{\omega}_j)} as follows. For more details see Weng and Yin (2018).  
#' \mjsdeqn{\widehat{\boldsymbol{\psi}}(\boldsymbol{\omega}_j)=n^{-1}\sum_{k=1}^n \exp( i \boldsymbol{\omega}_j^T\textbf{y}_k)\widehat{\textbf{Z}}_k, j=1,\cdots,t,}
#' where \mjseqn{\widehat{\textbf{Z}}_j=\boldsymbol{\Sigma}_{x}^{-1/2}(\textbf{x}_i-\overline{\textbf{x}})}. Now, let
#' \mjseqn{\textbf{a}(\boldsymbol{\omega}_j)=Real(\widehat{\boldsymbol{\psi}}(\boldsymbol{\omega}_j))}, and \mjseqn{\textbf{b}(\boldsymbol{\omega}_j)=Image(\widehat{\boldsymbol{\psi}}(\boldsymbol{\omega}_j))}. Then, 
#' \mjseqn{\widehat{\boldsymbol{\Psi}}= (\textbf{a}(\boldsymbol{\omega}_1),\textbf{b}(\boldsymbol{\omega}_1),\cdots,\textbf{a}(\boldsymbol{\omega}_t),\textbf{b}(\boldsymbol{\omega}_t))}, for some \eqn{t > 0}, and the population kernel matrix is
#' \mjseqn{\widehat{\textbf{V}} = \widehat{\boldsymbol{\Psi}}\widehat{\boldsymbol{\Psi}}^T}. Finally, use the \emph{d}-leading eigenvectors of \mjseqn{\widehat{\textbf{V}}} as an estimate for the central subspace.
#' 
#' \bold{Remark: }We use \emph{w} instead of \mjseqn{\boldsymbol{\omega}_1,\cdots,\boldsymbol{\omega}_t} in the \emph{invFM()} function. 
#' @references 
