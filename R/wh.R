...
#' @details 
#' \loadmathjax
#' The kernel density estimation of \mjseqn{f_{\textbf{x}}(\textbf{x})} at a fixed point \mjseqn{\textbf{x}_0} is defined as
#' \mjsdeqn{\widehat{f}_{\textbf{x}_0}(\textbf{x}_0)=(nh^p)^{-1}\sum_{\ell=1}^n G\left(\frac{\textbf{x}_0-\textbf{x}_{\ell}}{h}\right),}
#' where \mjseqn{G(\cdot)} is a Gaussian kernel function and `h' is the bandwidth of the 
