...
#' \mjsdeqn{\boldsymbol\psi(\boldsymbol\omega) =\int \frac{\partial}{\partial \textbf{x}}m(\textbf{x}) W(\textbf{x},\boldsymbol\omega)f(\textbf{x})d\textbf{x},}
#' where \mjseqn{W(\textbf{x},\boldsymbol\omega)} is said to be a nondegenerate kernel function. Set \mjseqn{W(\textbf{x},\boldsymbol\omega)=\exp(i\boldsymbol\omega^T\textbf{x})} for 
#' Fourier transformation (FM) method  and \mjseqn{W(\textbf{x},\boldsymbol\omega)=H(\textbf{x}-\boldsymbol\omega)=(2\pi\sigma_w^2)^{-p/2}\exp(-(\textbf{x}-\boldsymbol{\omega})^T(\textbf{x}-\boldsymbol\omega)/(2\sigma_w^2))} for convolution transformation (CM) method  
#' where \mjseqn{W(\textbf{x},\boldsymbol\omega)} is an absolutely integrable function. 
#' The candidate matrix to estimate the central mean subspace (CMS),
#' \mjsdeqn{\textbf{M}_{CMS}=\int \boldsymbol\psi(\boldsymbol\omega) \boldsymbol\psi(\boldsymbol\omega)^T K(\boldsymbol\omega)d\boldsymbol\omega, }
#' where \mjseqn{K(\boldsymbol{\omega})=(2\pi \sigma_w^2)^{-p/2}\exp{(-||\boldsymbol{\omega}||}/2\sigma_w^2)} under 'FM', and \mjseqn{K(\boldsymbol{\omega})=1} under `CM`. 
#' Here, \mjseqn{\sigma_w^2} is a tuning parameter and it refers as "tuning parameter for the predictor variables" and denoted by `wx` in all functions.
#' 
#' 
#' Let \mjseqn{\{T_v(y)=H(y,v),~ for~~ y,v\in \mathcal{R}\}} be the family of transformations for the response variable. That is, \mjseqn{v \in \mathcal{R}}, the mean 
#' response of \mjseqn{T_v(y)} is \mjseqn{m(\boldsymbol{\omega},v)=E[H(y,v)\vert \textbf{X}=\textbf{x}]}. Then, integral transformation for the gradient of
#' \mjseqn{m(\boldsymbol{\omega},v)} is defined as
#' \mjsdeqn{\boldsymbol{\psi}(\boldsymbol{\omega},v)=\int \frac{\partial}{\partial \textbf{x}}m(\textbf{x},v) W(\textbf{x},\boldsymbol{\omega})f(\textbf{x})d\textbf{x},}
#' where \mjseqn{W(\textbf{x},\boldsymbol{\omega})} is the define as above. Then, the 
#' candidate matrix for the central subspace (CS) is defined as
#' \mjsdeqn{\textbf{M}_{CS}=\int H(y_1,v)H(y_2,v)dv \int \boldsymbol{\psi}(\boldsymbol{\omega}) \bar{\boldsymbol{\psi}}(\boldsymbol{\omega})^T K(\boldsymbol{\omega})d\boldsymbol{\omega},}
#' where \mjseqn{K(\boldsymbol{\omega})} is the same as above, and \mjseqn{H(y,v)=(2\pi \sigma_t^2)^{-1/2}\exp(v^2/(2\sigma_t^2))} under  `FM`, 
#' and \mjseqn{H(y,v)=(2\pi \sigma_t^2)^{-1/2}\exp((y-v)^2/(2\sigma_t^2))} under `CM`.
#' Here \mjseqn{\sigma_t^2} is a tuning parameter and it refers as the "tuning parameter for the response variable" and is denote by `wy` in all functions.
#' 
