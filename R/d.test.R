#' Testing methods for selecting dimension of the central mean subspace. 
#' 
#' @description 
...
#' 
#' \mjseqn{~~~~~H_0: d=m~~ \text{vs}~~ H_a: d>m}.
#' 
#' Cook's test statistics (Cook, 1998; Weng and Yin, 2018): 
#' \mjsdeqn{\hat{\Lambda}=n\sum_{j=m+1}^{p}\hat{\lambda}_j,}
#' where \mjseqn{\lambda_j}'s are the eigenvalues of \eqn{\widehat{\textbf{V}}} where \eqn{\widehat{\textbf{V}}} is defined under \emph{invFM()} function. 
#' 
#' 
#' Scaled test statistic (Bentler and Xie, 2000):
#' \mjsdeqn{\overline{T}_m=[trace(\hat{\Omega}_n)/p^{\star}]^{-1}n\sum_{j=m+1}^{p}\hat{\lambda}_j \sim \mathcal{X}^2_{p^{\star}},}
#' where \mjseqn{\hat{\Omega}_n} is a covariance matrix (Bentler and Xie, 2000), and \mjseqn{p^{\star} = (p-m)(2t-m)}.
#' 
#' 
#' Adjusted test statistic (Bentler and Xie, 2000):
#' \mjsdeqn{\tilde{T}_m=[trace(\hat{\Omega}_n)/d^{\star}]^{-1}n\sum_{j=m+1}^{p}\hat{\lambda}_j \sim \mathcal{X}^2_{d^{\star}},}
#' where \mjseqn{\hat{\Omega}_n} is a covariance matrix (Bentler and Xie, 2000), and \mjseqn{d^{\star} = [trace(\hat{\Omega}_n)]^{2}/trace(\hat{\Omega}_n^2)} .
#' @export
#' @return The \emph{d.test()} returns a table of p-values for each test.
#'
#' @examples 
#' library(itdr)
#' data(PDB)
