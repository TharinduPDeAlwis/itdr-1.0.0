...
#' \mjseqn{p \times q}. Suppose \mjseqn{\mathcal{S}(\textbf{A})} and 
#' \mjseqn{\mathcal{S}(\textbf{B})} are the column subspaces of matrices
#' \bold{A} and \bold{B}, respectively. 
#' And, let \mjseqn{\lambda_i} 's with 
#' \mjseqn{1 \geq \lambda_1^2 \geq \lambda_2^2 \geq,\cdots,\lambda_p^2\geq 0}, 
#' are the eigenvalues of the matrix \mjseqn{\textbf{B}^T\textbf{A}\textbf{A}^T\textbf{B}}.
#' 
#' (Trace correlation, Hotelling, 1936) \mjsdeqn{\gamma=\sqrt{\frac{1}{p}\sum_{i=1}^{p}\lambda_i^2}}
#' 
#' (Vector correlation, Hooper, 1959) \mjsdeqn{\theta=\sqrt{\prod_{i=1}^{p}\lambda_i^2}}
#' @references
